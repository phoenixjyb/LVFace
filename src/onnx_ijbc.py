# Original code is from https://github.com/deepinsight/insightface
...
class AlignedDataSet(mx.gluon.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # output = img_1.astype(np.float32)
...
def extract(model_root, dataset):
    ...
    feat_mat = np.zeros(shape=(len(dataset), model.feat_dim * 2))
    ...
    data_loader = DataLoader(
        dataset, batch_size=batch_size, drop_last=False, num_workers=32, collate_fn=collate_fn, )
    ...
    for batch in data_loader:
        ...
        feat_mat[batch_size * num_iter: batch_size * num_iter + feat.shape[0], :] = feat
        num_iter += 1
        if num_iter % 50 == 0:
            print(num_iter)
    return feat_mat
...
def main(args):
    ...
    import csv
    print(tpr_fpr_table)
    with open(os.path.join(save_path, "{}.csv".format(args.target)), 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(tpr_fpr_table)
...
if __name__ == '__main__':
    ...
    # source_path = 'work_dirs_poolformer/'
    # model_paths = os.listdir(source_path)
    # os.environ["CUDA_VISIBLE_DEVICES"] = '3'
    # for model_path in model_paths[10:]:
    #     model_root = os.path.join(source_path, model_path)
